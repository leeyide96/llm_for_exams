...
def create_quiz_generation_chain(vectorstore,search_query, docs):
    ...
    llm = ChatVertexAI(model_name="gemini-1.5-flash-001",temperature=0.1,max_tokens=8192)
    ...
    prompt = PromptTemplate(
        ...
        8. Avoid questions that require the user to refer back to the file to get the full context. Questions with example like, Based on Dataset in the file, should be avoided.
        ...
        Do think twice before producing the output. NEVER INCLUDE ```json or ``` IN YOUR OUTPUT.
        """,
...
def create_summary_generation_chain(vectorstore, search_query, docs):
    ...
    retriever = vectorstore.as_retriever()
    prompt = PromptTemplate(    
        input_variables=["text", "conciseness", "layman", "topic"],
        template="""
            You are an expert academic tutor preparing students for exams. Given the academic context at the end, provide a comprehensive summary focusing on key concepts related.

            Please provide an academic summary that:
            1. Defines and explains the main concepts related to the topic which is on {topic}. If it is on All Topics, ensure that every main concepts are explained clearly.
            ...
            5. Ensure that the total words count must be less than 8100 regardless of conciseness
            ...
            [List every key concepts with {conciseness}.  {layman}]
            ...
            Summary:
            [Summarize the concepts explained and write down the key takeaways from the academic context]
